...
class robin_data:
    ...
    def get_stock_historicals_df(self, inputSymbols, span="week", bounds="regular"):
        ...
        res = rs.stocks.get_historicals(
            inputSymbols=inputSymbols, span=span, bounds=bounds
        )
        res_df = pd.DataFrame(res)
        res_df.index = pd.to_datetime(res_df["begins_at"]) + pd.Timedelta(
            hours=-4
        )  # convert to EST
    ...
    def get_crypto_historicals_df(
        self, inputSymbol="BTC", interval="15second", span="hour", bound="24_7"
    ):
    ...
    def plot_stock_historicals(
        self, stock_symbol, span, bounds, figsize=(20, 12), title="", subplots="MACD"
    ):
